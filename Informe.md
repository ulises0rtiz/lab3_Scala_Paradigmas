...
- **¿Qué ocurriría si no llamaran a cache() y cuántas veces se ejecutaría la descarga?**
  Spark utiliza evaluación perezosa (_lazy evaluation_). Esto significa que las transformaciones (como la descarga de feeds HTTP) no se ejecutan cuando se declaran, sino que se recalifican desde la raíz cada vez que el programa encuentra una _Acción_ terminal (como `.count()` o `.collect()`).
  En nuestro pipeline, tenemos dos acciones terminales principales: el conteo de posts válidos (`filteredPostsRDD.count()`) y la recolección final del ranking Map-Reduce (`sortedRDD.collect()`). Si no usáramos `.cache()` en el `filteredPostsRDD`, **la descarga HTTP de todos los feeds y el parseo de los JSON se ejecutarían dos veces** completas, una por cada acción solicitada, destruyendo el rendimiento del sistema y sobrecargando la red.

- **El antipatrón de usar collect() a mitad de camino:**
  Si llamáramos a `.collect()` después de extraer las entidades (paso a del Ejercicio 3) y antes de mapearlas (paso b), estaríamos cometiendo un error arquitectónico grave. `.collect()` es una acción que interrumpe la tubería distribuida y transfiere absolutamente todos los datos desde la memoria fragmentada de los Workers a la memoria singular del Driver. Si hiciéramos esto, perderíamos toda la capacidad de procesamiento distribuido; el resto de las operaciones (como el conteo y el ordenamiento) se ejecutarían como código Scala secuencial normal en el hilo único del Driver. En un entorno de Big Data real, el Driver colapsaría por falta de memoria (OutOfMemoryError).

- **Evaluación perezosa de cache():**
  La instrucción `.cache()` es en sí misma una declaración perezosa. Al escribir `rdd.cache()`, no se carga nada en memoria instantáneamente. El RDD se materializa y se almacena físicamente en la memoria RAM de los Workers **únicamente durante la ejecución de la primera Acción que lo requiere** (en nuestro caso, cuando se ejecuta `filteredPostsRDD.count()`). A partir de ese momento, las acciones subsiguientes leerán los datos directamente de la memoria caché.
